...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_nosey_parker_enable", Cmd_ReloadCfg, ADMFLAG_GENERIC, "Enable or disable");
...
public Action Cmd_ReloadCfg(int client, int args)
{
	...
	PrintToChat(client, "解控模式已 %s", flag[client] ? "开启" : "关闭");
...
public void Event_CHJ_Attack(Event hEvent, const char[] sEventName, bool bDontBroadcast)
{
	...
	RequestFrame(next_frame_kill, iAttacker);
}
...
public void Event_SmokerAttackFirst(Event hEvent, const char[] sEventName, bool bDontBroadcast)
{
	...
	if (flag[iVictim] && iAttacker > 0 && iVictim > 0) {
		// int checks = 0;
...
public Action CheckSurvivorState(Handle hTimer, ArrayStack hEventMembers)
{
	...
	if (!hEventMembers.Empty) {
		...
		if (flag[iVictim] && iVictim > 0 && iAttacker > 0) { //if players in game
...
void next_frame_kill(int userid)
{
