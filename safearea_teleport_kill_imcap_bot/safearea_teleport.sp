...
public void OnPluginStart() {
	...
	g_hCvarModesTog =		CreateConVar("st_modes_tog",	"1",	"Turn on the plugin in these game modes. 0=All, 1=Coop, 2=Survival, 4=Versus, 8=Scavenge. Add numbers together.", CVAR_FLAGS);

	g_hSafeAreaFlags =		CreateConVar("st_enable",		"1",	"Where is it enabled? (1=Safe Room, 2=Rescue Vehicle, 3=Both)", CVAR_FLAGS);
	g_hSafeAreaType =		CreateConVar("st_type",			"2",	"How to deal with players who have not entered the destination safe area (1=teleport, 2=slay)", CVAR_FLAGS);
	g_hSafeAreaTime =		CreateConVar("st_time",			"3",	"How many seconds to count down before processing", CVAR_FLAGS);
...
void OnStartTouch(const char[] output, int caller, int activator, float delay) {
	if (g_bIsSacrificeFinale || !g_bFinaleVehicleReady || !g_iSafeAreaTime || activator < 1 || activator > MaxClients || !IsClientInGame(activator) || GetClientTeam(activator) != 2 || !IsPlayerAlive(activator))
		return;
	...
	//PrintToChatAll("入: 终点内 %d 名玩家", reached);
...
void OnEndTouch(const char[] output, int caller, int activator, float delay) {
	if (g_bIsSacrificeFinale || !g_bFinaleVehicleReady || !g_iSafeAreaTime || activator < 1 || activator > MaxClients || !IsClientInGame(activator) || GetClientTeam(activator) != 2)
		return;
	...
	//PrintToChatAll("出: 终点内 %d 名玩家", reached);
	value = RoundToCeil(g_iMinSurvivorPercent / 100.0 * value);
	if (reached < value) {
		delete g_hTimer;
		return;
...
Action tmrCountdown(Handle timer) {
	//PrintToChatAll("men: %d %d", GetEntProp(g_LastDoor.id, Prop_Send, "m_eDoorState"), GetEntProp(g_LastDoor.entRef, Prop_Send, "m_eDoorState"));
	//0=关, 2=开, 1=正在打开 3=正在关闭
	if(g_LastDoor.id != -1 && GetEntProp(g_LastDoor.id, Prop_Send, "m_eDoorState") > 0) 
	{
		g_iCountdown = g_iSafeAreaTime;
		return Plugin_Continue;
	}
	//没倒地的电脑在外面, 或者 有特感在里面
	for (int i = 1; i <= MaxClients; i++) {
		if (IsClientInGame(i) && IsFakeClient(i) && IsPlayerAlive(i) && ((GetClientTeam(i) == 2 && !GetEntProp(i, Prop_Send, "m_isIncapacitated") && !IsPlayerInEndArea(i)) || (GetClientTeam(i) == 3 && IsPlayerInEndArea(i)))) {
			g_iCountdown = g_iSafeAreaTime;
			return Plugin_Continue;
		}	
	}
	//PrintToChatAll("%d 秒后干掉", g_iCountdown);
	if (g_iCountdown > 0) {
		g_iCountdown--;
		...
		RequestFrame(Perform, g_iSafeAreaType);
		g_hTimer = null;
		return Plugin_Stop;
	}
...
void Perform(int type) {
	switch (type) {
		...
		case 2: {
			for (int i = 1; i <= MaxClients; i++) {
				if (IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i) && IsFakeClient(i) && GetEntProp(i, Prop_Send, "m_isIncapacitated") && !IsPlayerInEndArea(i, false))
...
void InitData() {
	...
	//DynamicDetour dDetour = DynamicDetour.FromConf(hGameData, "CFirstAidKit::ShouldStartAction");
	//if(!dDetour)
	//	SetFailState("Failed to find CFirstAidKit::ShouldStartAction signature.");
	//if(!dDetour.Enable(Hook_Post, MedkitStartAction)) 
	//	SetFailState("Failed to find CFirstAidKit::ShouldStartAction signature.");
	//delete dDetour;
	...
	if (!PrepSDKCall_SetFromConf(hGameData, SDKConf_Signature, "CTerrorPlayer::StopBeingRevived"))
		SetFailState("Bad siganture \"CTerrorPlayer::StopBeingRevived\"");
	PrepSDKCall_AddParameter(SDKType_Bool, SDKPass_ByRef);
	if (!(g_hSDK_StopBeingRevived = EndPrepSDKCall()))	
		SetFailState("Failed to create SDKCall: \"CTerrorPlayer::StopBeingRevived\"");
