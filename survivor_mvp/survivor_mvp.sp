...
public void OnPluginStart() {
	g_cvPrintTime = CreateConVar("sm_mvp_time", "0", "轮播时间间隔", FCVAR_NOTIFY, true, 0.0);
...
void Event_RoundEnd(Event event, const char[] name, bool dontBroadcast) {
	...
	CreateTimer(5.0, DontShowTwoTimeInRow);
	OnMapEnd();
}

Action DontShowTwoTimeInRow(Handle timer)
{
	g_bShowOne = false;
	return Plugin_Handled;
}
...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast) {
	...
	{
		SetIncrementFragCount(victim, g_esData[victim].killSI + 1);
		return;
	}
	...
	switch (class) {
		case 1, 2, 3, 4, 5, 6: {
			...
			SetIncrementFragCount(attacker, g_esData[attacker].killSI);
		}
		/*
		case 8:
			g_esData[attacker].killSI++;*/
...
void PrintStatistics() {
	...
	SortIntegers(dataSort, count, Sort_Descending);
	int dmgSIFLen = !count ? 1 : IntToString(dataSort[0], str, sizeof str);
	...
	for (i = 0; i < infoMax; i++) {
		...
		if(killSI == 0 && dmgSI == 0 && killCI == 0)
		{
			continue;
		}

		strcopy(buffer, sizeof buffer, "\x01特感: ");
		...
		len = strlen(buffer);
		strcopy(buffer[len], sizeof buffer - len, "\x01[");
		numSpace = dmgSIFLen - IntToString(dmgSI, str, sizeof str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		len = strlen(buffer);
		Format(buffer[len], sizeof buffer - len, "\x04%s", str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		...
		len = strlen(buffer);
		strcopy(buffer[len], sizeof buffer - len, "\x01] \x01丧尸: ");
		...
		len = strlen(buffer);
		Format(buffer[len], sizeof buffer - len, " \x04%N", client);
	...
	killSI = g_esData[client].killSI;
	if (killSI > 0)
		PrintToChatAll("\x01MVP: \x05%N \x01伤害: \x05%d\x01(\x04%d%%\x01) 击杀: \x05%d\x01(\x04%d%%\x01)", client, dmgSI, RoundToNearest(float(dmgSI) / float(g_iTotaldmgSI) * 100.0), killSI, RoundToNearest(float(killSI) / float(g_iTotalkillSI) * 100.0));
	...
	teamFF = g_esData[client].teamFF;
	if (teamFF >= 30)
		PrintToChatAll("\x01黑枪之王: \x05%N \x01友伤: \x05%d\x01(\x04%d%%\x01)", client, teamFF, RoundToNearest(float(teamFF) / float(g_iTotalFF) * 100.0));
...
void PrintTankStatistics(int tank) {
	...
	CPrintToChatAll("{default}[{green}%s{default}] {olive}%N {default}伤害承受: {blue}%d", IsFakeClient(tank) ? "AI" : "PZ", tank, g_esData[tank].totalTankDmg);
	...
	for (i = 0; i < length; i++) {
		...
		strcopy(buffer, sizeof buffer, "{default}[");
		numSpace = dmgLen - IntToString(damage, str, sizeof str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		len = strlen(buffer);
		Format(buffer[len], sizeof buffer - len, "{orange}%s", str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		...
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		len = strlen(buffer);
		Format(buffer[len], sizeof buffer - len, "{blue}%s%%", str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);

		len = strlen(buffer);
		strcopy(buffer[len], sizeof buffer - len, "{default}) 拳: ");
		...
		len = strlen(buffer);
		strcopy(buffer[len], sizeof buffer - len, " {default}饼: ");
		...
		len = strlen(buffer);
		strcopy(buffer[len], sizeof buffer - len, " {default}铁: ");
...
void SetIncrementFragCount(int client, int count)
{
	SetEntProp(client, Prop_Data, "m_iFrags", count);
	SetEntData(client, 7368, count);
}